...
CMD ["sh","-c","gunicorn --workers=$WORKERS --threads=2 --worker-class=gthread --config=/app/gunicorn.conf.py --timeout=60 --keep-alive=30 --bind=0.0.0.0:$SNOWFACE_PORT 'app:create_app()'"]
