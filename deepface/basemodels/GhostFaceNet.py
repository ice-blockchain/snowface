...
def load_model():
    ...
    if os.path.isfile(output) is not True:
        print(f"Pre-trained weights is downloaded from {PRETRAINED_WEIGHTS} to {output}")
        gdown.download(PRETRAINED_WEIGHTS, output, quiet=False)
        print(f"Pre-trained weights is just downloaded to {output}")
